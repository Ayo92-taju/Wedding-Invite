...
export async function notifyParty({ origin, partyName, primaryName, email, phone, members, declined = false }) {
  const out = { emailed: false, sms: null, whatsapp: [] }
  const httpsOrigin = /^https:\/\//i.test(origin || '')
  const passUrl = (m) => `${origin}/api/pass-image?c=${encodeURIComponent(m.inviteCode)}`
  ...
  if (to.includes('@')) {
    try {
      ...
      const { subject, html } = partyConfirmationEmail({
        ...
        members: members.map((m) => ({ ...m, passImageUrl: httpsOrigin ? passUrl(m) : undefined })),
      })
      if (resend) {
        ...
        console.log(`[EMAIL SIMULATOR] Party confirmation → ${to} (${members.length} passes)`)
      }
    } catch (err) {
  ...
  if (phone) {
    try {
      out.sms = await sendSms(phone, passMessage({ partyName, members, siteUrl: origin }))
    } catch (err) {
